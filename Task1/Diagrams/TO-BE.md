...
System_Boundary(ad_sys, "RTB System") {
  Container(biddingService, "RTB Service", "Go", "Приём bid request, идентификация пользователя, подбор кандидатов, определение победителя аукциона")
  ContainerDb(bid_postgres, "PostgreSQL (Primary)", "PostgreSQL", "Кампании, ставки, правила, конфигурации")
  ContainerDb(bid_postgres_read, "PostgreSQL (Read Replica)", "PostgreSQL", "Кампании, ставки, правила, конфигурации")
  ContainerDb(bid_redis_read, "Redis", "Redis", "Чтение кампаний, таргетинга, ставок, правил, конфигурации")
...
System_Boundary(delivery_sys, "Delivery System") {
  ...
  ContainerDb(del_postgres_read, "PostgreSQL (Replica)", "PostgreSQL", "Кампании, креативы")
  ContainerDb(del_redis_read, "Redis", "Redis", "Кампании, креативы")

  Rel(deliveryService, del_postgres, "Запись кампаний, креативов")
  Rel(deliveryService, del_postgres_read, "Чтение кампаний, креативов (при cache miss)")
...
Rel(dspGateway, biddingService, "Маршрутизация bid requests", "HTTPS")
...
Rel(targ_topic, biddingService, "Событие изменения таргетинга", "Event")

Rel(campaignService, camp_topic, "Событие изменения кампании", "Event")
Rel(camp_topic, biddingService, "Событие изменения кампании", "Event")
...
note left of biddingService
  **Bounded Context: Аукцион**
  ...
  - Получает обновления кампаний, таргетинга и ставок через Kafka (async)
  - Своя PostgreSQL (изоляция от блокировок)
  - Синхронно предоставляет результаты аукциона
  - Горизонтальное масштабирование независимо
end note
